...
// ═══════════════════════════════════════
// فاز ۱: سیستم احراز هویت
// ═══════════════════════════════════════
require_once get_template_directory() . '/inc/auth-system.php';

/**
 * جلوگیری از دسترسی ادمین برای کاربران عادی
 */
...
/**
 * حذف admin bar برای کاربران عادی
 */
...
/**
 * فیلتر آواتار وردپرس → آواتار سفارشی
 */
add_filter('get_avatar_url', 'novel_filter_avatar_url', 10, 3);
function novel_filter_avatar_url($url, $id_or_email, $args) {
    $user_id = 0;
...
/**
 * غیرفعال کردن ایمیل‌های پیش‌فرض وردپرس
 * (چون ایمیل سفارشی خودمان ارسال می‌شود)
 */
...
// جلوگیری از ارسال لینک در متن نظرات
function fn_block_links_in_comments( $commentdata ) {
    // اگر کاربر ادمین بود، به او اجازه بده (اختیاری)
    if ( current_user_can( 'administrator' ) ) {
        return $commentdata;
    }

    // بررسی وجود http، https یا www در متن نظر
    $content = $commentdata['comment_content'];
    $pattern = '/\b(?:https?:\/\/|www\.)\S+\b/i';

    if ( preg_match( $pattern, $content ) ) {
        wp_die( 'خطا: ارسال لینک (URL) در نظرات مجاز نمی‌باشد. لطفاً لینک را حذف کرده و مجدداً تلاش کنید.' );
    }

    return $commentdata;
}
add_filter( 'preprocess_comment', 'fn_block_links_in_comments' );
...
// حذف فیلد وب‌سایت از فرم نظرات یادیدگاه
function fn_remove_website_field( $fields ) {
    if ( isset( $fields['url'] ) ) {
        unset( $fields['url'] );
    }
    return $fields;
}
add_filter( 'comment_form_default_fields', 'fn_remove_website_field' );
...
// استایل‌دهی سفارشی به صفحه خطاهای وردپرس (wp_die)  در بخش کامنت ها اگر زیاد ارسال بشه wp-comments-post.php
function fn_custom_wp_die_handler( $message, $title = '', $args = array() ) {
    $defaults = array( 'response' => 500 );
    $r = wp_parse_args($args, $defaults);

    // اگر درخواست AJAX بود، خروجی جیسون بده (برای فرم‌های ایجکس)
    ...
    <head>
        ...
        <title><?php echo $title ? esc_html($title) : 'خطا در عملیات'; ?></title>
        <style>
            body {
                background: #f4f6f9;
    ...
    <body>
        <div class="fn-error-card">
            <div class="fn-error-icon">⚠️</div>
            <h1 class="fn-error-title"><?php echo $title ? esc_html($title) : 'مشکلی پیش آمد!'; ?></h1>
            <div class="fn-error-message">
                <?php echo wp_kses_post( $message ); ?>
            </div>
            <a href="javascript:history.back()" class="fn-back-btn">بازگشت و اصلاح</a>
        </div>
    </body>
    </html>
    <?php
...
// اتصال هندلر سفارشی به وردپرس
add_filter( 'wp_die_handler', function() {
    return 'fn_custom_wp_die_handler';
});